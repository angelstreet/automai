...
def execute():
    try:
        ...
        print(f"[execute] Created script folder: {script_folder_path}", file=sys.stderr)
        ...
        # Copy the script file to the script folder for uploading later
        script_file_name = os.path.basename(full_script_path)
        temp_script_path = os.path.join(script_folder_path, script_file_name)
        try:
            shutil.copy2(full_script_path, temp_script_path)
            print(f"DEBUG: Copied script file to script folder: {temp_script_path}", file=sys.stderr)
            # Set the script file path in env vars for future reference
            env_vars['SCRIPT_FILE_PATH'] = temp_script_path
        except Exception as e:
            print(f"DEBUG: Failed to copy script file to script folder: {str(e)}", file=sys.stderr)

        # Set script folder as an environment variable for scripts to use if needed
        env_vars['SCRIPT_TEMP_FOLDER'] = script_folder_path
        ...
        try:
            stdout_file = os.path.join(script_folder_path, "stdout.txt")
            with open(stdout_file, 'w') as f:
                f.write(result.get("output", ""))
            
            stderr_file = os.path.join(script_folder_path, "stderr.txt")
...
def finalize_job():
    ...
    # Execute upload_and_report.py to scan uploadFolder and upload files
    upload_folder = os.path.join(os.getcwd(), 'uploadFolder')
    job_folder_name = f"{created_at.split('T')[0].replace('-', '')}_{created_at.split('T')[1].split('.')[0].replace(':', '')}_{job_id}"
    job_folder_path = os.path.join(upload_folder, job_folder_name)
    ...
    try:
        upload_script_path = os.path.join(os.getcwd(), 'upload_and_report.py')
