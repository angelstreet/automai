      ...
      try {
        ...
        if (!fs.existsSync(scriptPath)) {
          ...
          for (const potentialPath of potentialPaths) {
            console.log(
              `[@runner:generateAndUploadReport] Checking for script at: ${potentialPath}`,
            );
            if (fs.existsSync(potentialPath)) {
              scriptPath = potentialPath;
              console.log(`[@runner:generateAndUploadReport] Found script at: ${scriptPath}`);
              break;
            }
          }
        }
        ...
        if (fs.existsSync(scriptPath)) {
          ...
          console.log(
            `[@runner:generateAndUploadReport] Copied script ${scriptName} to temporary location: ${tempScriptPath}`,
          );
          ...
          console.log(
            `[@runner:generateAndUploadReport] Cleaned up temporary script file: ${tempScriptPath}`,
          );
        } else {
          console.log(
            `[@runner:generateAndUploadReport] Script file not found at any checked path. Unable to upload script file.`,
          );
        }
        ...
        if (fs.existsSync(tempScriptPath)) {
          fs.unlinkSync(tempScriptPath);
          console.log(
            `[@runner:generateAndUploadReport] Cleaned up temporary script file after error: ${tempScriptPath}`,
          );
        }
      }
    } else {
