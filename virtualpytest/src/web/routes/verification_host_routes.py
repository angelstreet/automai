...
def host_text_auto_detect():
    """Host-side endpoint to crop image area and perform OCR text detection."""
    try:
        data = request.get_json()
        source_filename = data.get('source_filename')
        area = data.get('area')
        ...
        print(f"[@route:host_text_auto_detect] Text auto-detection request: {source_filename} with area: {area}")
        print(f"[@route:host_text_auto_detect] Model: {model}, Image filter: {image_filter}")
        ...
        # Create filename for text detection preview
        ...
        # Import and use existing cropping function
        try:
            from controllers.verification.image import crop_reference_image, apply_image_filter
            
            # Crop the image to the specified area
            success = crop_reference_image(source_path, target_path, area, create_filtered_versions=False)
            ...
            if image_filter and image_filter != 'none':
                print(f"[@route:host_text_auto_detect] Applying {image_filter} filter for OCR")
            ...
            # Perform OCR on the cropped (and optionally filtered) image
            try:
                import pytesseract
                import cv2
                ...
                try:
                    ...
                    for i in range(len(ocr_data['text'])):
                        ...
                        if text and confidence >= 30:  # Reasonable threshold for auto-detection
                    ...
                    print(f"[@route:host_text_auto_detect] OCR results: '{detected_text}' (confidence: {avg_confidence:.1f}%)")
                    
                except Exception as ocr_error:
                    print(f"[@route:host_text_auto_detect] Detailed OCR failed, trying fallback: {ocr_error}")
                    # Fallback to simple string method
                    detected_text = pytesseract.image_to_string(img, lang='eng').strip()
                    avg_confidence = 70.0 if detected_text else 0.0
                    avg_font_size = 12.0
                
                # Try language detection if we have enough text
                    ...
                    try:
                        ...
                        if lang_probs and len(lang_probs) > 0:
                            ...
                            lang_to_tesseract = {
                                ...
                                'es': 'spa', 'pt': 'por', 'nl': 'nld', 'ru': 'rus'
                            }
                            detected_language = lang_to_tesseract.get(lang_code, 'eng')
                            
                            # Language names for display
                            lang_names = {
                                'eng': 'English', 'fra': 'French', 'deu': 'German', 'ita': 'Italian',
                                'spa': 'Spanish', 'por': 'Portuguese', 'nld': 'Dutch', 'rus': 'Russian'
                            }
                            detected_language_name = lang_names.get(detected_language, 'English')
                            
                            print(f"[@route:host_text_auto_detect] Language detected: {detected_language_name} ({language_confidence:.3f})")
                    
                    except ImportError:
                        print(f"[@route:host_text_auto_detect] langdetect not available")
                    except Exception as lang_error:
                        print(f"[@route:host_text_auto_detect] Language detection error: {lang_error}")
                
                # Return URL path for the cropped preview image
                preview_url = f'/stream/captures/cropped/{target_filename}'
                print(f"[@route:host_text_auto_detect] Text auto-detection successful: {preview_url}")
                ...
                return jsonify({
                    ...
                    'message': f'Text auto-detection completed: "{detected_text[:50]}{"..." if len(detected_text) > 50 else ""}"'
                })
                
            except ImportError as e:
                print(f"[@route:host_text_auto_detect] OCR import error: {e}")
                return jsonify({
                    'success': False,
                    'error': 'OCR utilities not available (pytesseract required)'
                }), 500
            ...
            return jsonify({
                'success': False,
                'error': 'Image processing utilities not available'
            }), 500
        ...
        return jsonify({
            'success': False,
            'error': f'Host text auto-detection error: {str(e)}'
        }), 500
...
def host_save_text_resource():
    """Save text reference to resources directory and update git repository."""
    try:
        data = request.get_json()
        reference_name = data.get('reference_name')
        model = data.get('model')
        ...
        print(f"[@route:host_save_text_resource] Saving text resource: {reference_name} for model: {model}")
        ...
        if not reference_name or not model or not area or not text:
            return jsonify({
                'success': False,
                'error': 'reference_name, model, area, and text are required'
            }), 400
        ...
        try:
            ...
            # Use simple relative path to git repository root
            repo_path = "../../../"
            print(f"[@route:host_save_text_resource] Using git repository: {repo_path}")
            
            # Open the git repository
            repo = git.Repo(repo_path)
            
            # Step 1: Git pull FIRST - before modifying any files
            print(f"[@route:host_save_text_resource] Executing git pull to get server changes...")
            try:
                origin = repo.remotes.origin
                origin.pull()
                print(f"[@route:host_save_text_resource] Git pull successful")
            except Exception as pull_error:
                print(f"[@route:host_save_text_resource] Git pull failed: {str(pull_error)}")
                # Continue anyway - we'll try to commit our changes
        
        except ImportError:
            print(f"[@route:host_save_text_resource] GitPython not available, skipping git operations")
        except Exception as e:
            print(f"[@route:host_save_text_resource] Git operation error: {str(e)} - continuing anyway")
        
        # Step 2: Update resource.json with text reference data
        try:
            # Load existing resource.json or create new structure
            if os.path.exists(resource_json_path):
                with open(resource_json_path, 'r') as f:
            ...
            # Create new text resource entry
            new_resource = {
                "name": reference_name,
                "model": model,
                "path": f"resources/{model}/{reference_name}.png",  # Keep path for consistency, even though no image file
                "created_at": datetime.now().isoformat(),
            ...
            print(f"[@route:host_save_text_resource] Updated resource.json with new text resource: {reference_name}")
            
        except Exception as e:
            print(f"[@route:host_save_text_resource] Failed to update resource.json: {str(e)}")
            return jsonify({
                'success': False,
                'error': f'Failed to update resource.json: {str(e)}'
            }), 500
        ...
        try:
            ...
            # Add resource.json file to git index
            print(f"[@route:host_save_text_resource] Adding resource.json to git...")
            try:
                # Add the resource.json file (relative to repo root)
                config_git_path = 'virtualpytest/src/config/resource/resource.json'
                repo.index.add([config_git_path])
                
                print(f"[@route:host_save_text_resource] File added to git index: {config_git_path}")
            except Exception as add_error:
                print(f"[@route:host_save_text_resource] Git add failed: {str(add_error)}")
            
            # Git commit
            commit_message = f"save text resource {reference_name} for model {model}"
            print(f"[@route:host_save_text_resource] Committing changes...")
            try:
                repo.index.commit(commit_message)
                print(f"[@route:host_save_text_resource] Git commit successful")
            except Exception as commit_error:
                print(f"[@route:host_save_text_resource] Git commit failed: {str(commit_error)}")
            
            # Git push - requires token authentication
            print(f"[@route:host_save_text_resource] Pushing to remote...")
            try:
                # Require GitHub token for authentication
                github_token = os.environ.get('GITHUB_TOKEN')
                if not github_token:
                    print(f"[@route:host_save_text_resource] No GITHUB_TOKEN found - skipping push")
                    print(f"[@route:host_save_text_resource] Changes committed locally only")
                else:
                    print(f"[@route:host_save_text_resource] Using token authentication for push")
                    origin = repo.remotes.origin
                    original_url = origin.url
                    ...
                    if original_url.startswith('https://github.com/'):
                        ...
                        try:
                            origin.push()
                            print(f"[@route:host_save_text_resource] Git push successful")
                        finally:
                            # Restore original URL
                            origin.set_url(original_url)
                    else:
                        print(f"[@route:host_save_text_resource] Remote URL not supported for token auth: {original_url}")
                        
            except Exception as push_error:
                print(f"[@route:host_save_text_resource] Git push failed: {str(push_error)}")
                print(f"[@route:host_save_text_resource] Changes committed locally only")
            
            print(f"[@route:host_save_text_resource] Git operations completed")
            
        except Exception as e:
            print(f"[@route:host_save_text_resource] Git operation error: {str(e)} - continuing anyway")
        
        print(f"[@route:host_save_text_resource] Text resource saved successfully: {reference_name}")
        
        return jsonify({
            'success': True,
            'message': f'Text resource "{reference_name}" saved successfully'
        })
        ...
        return jsonify({
            'success': False,
            'error': f'Host save text resource error: {str(e)}'
        }), 500 