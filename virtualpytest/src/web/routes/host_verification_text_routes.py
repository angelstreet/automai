...
def execute_text_verification():
    ...
    try:
        ...
        # Execute verification using specific text verification controller
        from src.utils.host_utils import get_controller
        
        verification_controller = get_controller('device1', 'verification_text')
        if not verification_controller:
            return jsonify({
                'success': False,
                'error': 'Text verification controller not available'
            }), 500
