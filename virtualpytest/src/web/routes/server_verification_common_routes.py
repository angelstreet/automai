...
# =====================================================
# MONITORING ENDPOINTS (PROXY TO HOST IMAGE CONTROLLER)
# =====================================================

@server_verification_common_bp.route('/image/getLatestFrames', methods=['POST'])
def get_latest_monitoring_frames():
    """Proxy request to get latest frames from HDMI capture folder"""
    try:
        print("[@route:server_verification_common:get_latest_monitoring_frames] Proxying get latest frames request")
        
        # Get request data
        request_data = request.get_json() or {}
        
        # Proxy to host image verification endpoint for monitoring frames
        response_data, status_code = proxy_to_host('/host/verification/image/getLatestFrames', 'POST', request_data, timeout=30)
...
@server_verification_common_bp.route('/image/analyzeFrame', methods=['POST'])
def analyze_monitoring_frame():
    """Proxy request to analyze frame with AI monitoring"""
    try:
        print("[@route:server_verification_common:analyze_monitoring_frame] Proxying frame analysis request")
        
        # Get request data
        request_data = request.get_json() or {}
        
        # Proxy to host image verification endpoint for frame analysis
        response_data, status_code = proxy_to_host('/host/verification/image/analyzeFrame', 'POST', request_data, timeout=30)
