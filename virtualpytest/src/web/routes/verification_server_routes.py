...
def save_text_reference():
    ...
    try:
        ...
        print(f"[@route:save_text_reference] Forwarding text save request to host: {name} for model: {model}")
        ...
        if not name or not model or not text:
            return jsonify({
                'success': False,
                'error': 'Missing required parameters: name, model, and text are all required'
            }), 400
        ...
        save_payload = {
            'name': name,  # Use 'name' to match client expectations
            'model': model,
            'text': text,
            'font_size': font_size,
        ...
        try:
            ...
            if host_result.get('success'):
                ...
                return jsonify({
                    'success': True,
                    'message': f'Text reference saved to git repository: {name}',
                    'public_url': full_public_url,
