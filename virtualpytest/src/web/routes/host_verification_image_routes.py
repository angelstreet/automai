...
@verification_image_host_bp.route('/getLatestFrames', methods=['POST'])
def get_latest_monitoring_frames():
    """Get latest frames from HDMI capture folder for monitoring"""
    try:
        data = request.get_json() or {}
        device_id = data.get('device_id', 'device1')
        count = data.get('count', 180)  # Default 3 minutes at 1fps
        
        print(f"[@route:host_verification_image:get_latest_frames] Getting latest {count} frames for device: {device_id}")
        ...
        # Use image_monitoring helper to get latest frames
        from src.controllers.verification.image_monitoring import get_latest_frames
        result = get_latest_frames(image_controller, count)
        
        return jsonify(result)
        
    except Exception as e:
        print(f"[@route:host_verification_image:get_latest_frames] Error: {str(e)}")
        return jsonify({
            'success': False,
            'error': f'Get latest frames error: {str(e)}'
        }), 500

@verification_image_host_bp.route('/analyzeFrame', methods=['POST'])
def analyze_monitoring_frame():
    """Analyze frame with AI monitoring"""
    try:
        data = request.get_json() or {}
        device_id = data.get('device_id', 'device1')
        ...
        print(f"[@route:host_verification_image:analyze_frame] Analyzing frame {filename} for device: {device_id}")
        ...
        # Use image_monitoring helper for AI analysis
        from src.controllers.verification.image_monitoring import (
            analyze_blackscreen, 
            analyze_freeze, 
            analyze_subtitles_and_errors,
            detect_language
        )
        
        # Build full path to image in HDMI capture folder
        capture_folder = image_controller.capture_folder
        ...
        freeze = analyze_freeze(image_path, image_controller)
        subtitles, errors = analyze_subtitles_and_errors(image_path)
        language = detect_language(subtitles) if subtitles else 'unknown'
        ...
        print(f"[@route:host_verification_image:analyze_frame] Error: {str(e)}")
        return jsonify({
            'success': False,
