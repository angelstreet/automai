...
@host_control_bp.route('/initialize-host-device', methods=['POST'])
def initialize_host_device():
    """Initialize host device object in Flask app context"""
    try:
        print(f"[@route:initialize_host_device] Starting host device initialization")
        
        # Check if already initialized
        existing_host_device = getattr(current_app, 'my_host_device', None)
        if existing_host_device:
            print(f"[@route:initialize_host_device] Host device already initialized")
            return jsonify({
                'success': True,
                'message': 'Host device already initialized',
                'host_device_summary': {
                    'host_name': existing_host_device.get('host_name'),
                    'device_name': existing_host_device.get('device_name'),
                    'device_model': existing_host_device.get('device_model'),
                    'controller_objects_keys': list(existing_host_device.get('controller_objects', {}).keys())
                }
            })
        
        # Get the host device object from the server by re-registering
        print(f"[@route:initialize_host_device] Re-registering with server to get host device object")
        
        # Import the registration function
        import sys
        import os
        
        # Add the utils path to import hostUtils
        ...
        from hostUtils import register_host_with_server
        
        # Call registration function which should populate the host device object
        registration_result = register_host_with_server()
        
        # Check if host device is now initialized
        host_device = getattr(current_app, 'my_host_device', None)
        
        if host_device:
            print(f"[@route:initialize_host_device] Successfully initialized host device")
            return jsonify({
                'success': True,
                'message': 'Host device initialized successfully',
                'host_device_summary': {
                    'host_name': host_device.get('host_name'),
                    'device_name': host_device.get('device_name'),
                    'device_model': host_device.get('device_model'),
            ...
            print(f"[@route:initialize_host_device] Failed to initialize host device")
            return jsonify({
                'success': False,
                'error': 'Failed to initialize host device object after registration'
            }), 500
            
    except Exception as e:
        print(f"[@route:initialize_host_device] Error: {str(e)}")
        import traceback
        traceback.print_exc()
        return jsonify({
            'success': False,
            'error': f'Error initializing host device: {str(e)}'
        }), 500