...
export async function executeEdgeActions(
  ...
  console.log(
    `[@utils:navigationUtils:executeEdgeActions] Starting execution of ${actions.length} actions`,
  );
  ...
  for (let i = 0; i < actions.length; i++) {
    ...
    console.log(
      `[@utils:navigationUtils:executeEdgeActions] Executing action ${i + 1}/${actions.length}: ${action.label}`,
    );
    ...
    try {
      ...
      const result =
        await selectedHostDevice.controllerProxies.remote.executeAction(actionToExecute);
      ...
      if (!success && retryActions.length > 0) {
        console.log(
          `[@utils:navigationUtils:executeEdgeActions] Main action failed, executing ${retryActions.length} retry actions`,
        );

        for (let j = 0; j < retryActions.length; j++) {
          const retryAction = retryActions[j];
          console.log(
            `[@utils:navigationUtils:executeEdgeActions] Executing retry action ${j + 1}/${retryActions.length}: ${retryAction.label}`,
          );
          ...
          try {
            ...
            const retryResult =
              await selectedHostDevice.controllerProxies.remote.executeAction(retryActionToExecute);
            const retrySuccess = retryResult.success || false;
            const retryMessage =
              retryResult.message || (retrySuccess ? '✅ Retry Success' : '❌ Retry Failed');

            results.push(`Retry ${j + 1}: ${retryMessage}`);

            // Update retry action results
            const newRetryResults = updateLastRunResults(
              retryAction.last_run_result || [],
              retrySuccess,
            );
            updatedRetryActions[j] = {
              ...updatedRetryActions[j],
              last_run_result: newRetryResults,
            };
            ...
            console.error(
              `[@utils:navigationUtils:executeEdgeActions] Retry action ${j + 1} failed:`,
              retryError,
            );
  ...
  console.log(
    `[@utils:navigationUtils:executeEdgeActions] Execution completed. Results: ${results.length}`,
  );
