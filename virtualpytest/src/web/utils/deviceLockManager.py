...
def lock_device_in_registry(device_id: str, session_id: str = "default-session") -> bool:
    ...
    Args:
        device_id: The device/host ID to lock
        session_id: Session ID of the user locking the device
        
    Returns:
        bool: True if successfully locked, False if already locked or device not found
    """
    try:
...
def unlock_device_in_registry(device_id: str, session_id: Optional[str] = None) -> bool:
    ...
    Args:
        device_id: The device/host ID to unlock
    ...
    try:
        ...
        if not device.get('isLocked', False):
        ...
        if session_id and device.get('lockedBy') != session_id:
            locked_by = device.get('lockedBy', 'unknown')
        ...
        device.pop('lockedBy', None)
        device.pop('lockedAt', None)
...
def is_device_locked_in_registry(device_id: str) -> bool:
    ...
    Args:
        device_id: The device/host ID to check
    ...
    try:
        ...
        device = connected_clients[device_id]
        return device.get('isLocked', False)
...
def cleanup_expired_locks(timeout_seconds: int = 300) -> int:
    ...
    try:
        ...
        for device_id, device in connected_clients.items():
            if device.get('isLocked', False):
                locked_at = device.get('lockedAt', 0)
...
def force_unlock_device(device_id: str) -> bool:
    ...
    Args:
        device_id: The device/host ID to force unlock
