...
def get_all_navigation_trees(team_id: str) -> List[Dict]:
    ...
    try:
        result = supabase.table('navigation_trees').select(
            'id', 'name', 'userinterface_id', 'team_id', 'root_screen_id', 'description', 'created_at', 'updated_at',
        ...
        print(f"❌ Error retrieving navigation trees: {e}")
        return []

def get_navigation_tree(tree_id: str, team_id: str) -> Optional[Dict]:
    """Retrieve a specific navigation tree by ID."""
    supabase = get_supabase_client()
    ...
    try:
        result = supabase.table('navigation_trees').select(
            'id', 'name', 'userinterface_id', 'team_id', 'root_screen_id', 'description', 'created_at', 'updated_at',
            'userinterfaces(id, name, models)'
        ).eq('id', tree_id).eq('team_id', team_id).single().execute()
        ...
        print(f"❌ Error retrieving navigation tree {tree_id}: {e}")
        return None
...
def create_navigation_tree(tree_data: Dict, team_id: str) -> Optional[Dict]:
    ...
    try:
        ...
        insert_data = {
            'name': tree_data['name'],
            'userinterface_id': tree_data['userinterface_id'],
            'team_id': team_id,
        ...
        print(f"❌ Error creating navigation tree: {e}")
        return None
...
def update_navigation_tree(tree_id: str, tree_data: Dict, team_id: str) -> Optional[Dict]:
    ...
    try:
        ...
        print(f"❌ Error updating navigation tree {tree_id}: {e}")
        return None
