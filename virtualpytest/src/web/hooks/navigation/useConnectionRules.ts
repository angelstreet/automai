    ...
    rules: [
      {
        ...
        description:
          'Left/Right handles create sibling relationships - nodes share the same parent',
        example: 'home_tvguide ↔ home_apps (both have parent: [home])',
      },
      {
        name: 'Vertical Connections (Parent-Child)',
        description:
          'Top/Bottom handles create hierarchical relationships - target becomes child of source',
...
const establishConnectionRules = (
  ...
  const isVertical =
    params.sourceHandle?.startsWith('top') ||
    params.sourceHandle?.startsWith('bottom') ||
    params.targetHandle?.startsWith('top') ||
    params.targetHandle?.startsWith('bottom');

  // Check for HORIZONTAL connections (left/right handles)
  const isHorizontal =
    params.sourceHandle?.startsWith('left') ||
    params.sourceHandle?.startsWith('right') ||
    params.targetHandle?.startsWith('left') ||
    params.targetHandle?.startsWith('right');
  ...
  if (isVertical) {
    ...
    if (!sourceIsOrphan && !targetIsOrphan) {
      ...
      console.log(
        '[@hook:establishConnectionRules] VERTICAL: Both nodes have parents - preserving existing relationships',
      );
      return {
        isAllowed: true,
    ...
    if (sourceIsOrphan && targetIsOrphan) {
      ...
      if (sourceIsMenu && !targetIsMenu) {
        ...
        console.log(
          '[@hook:establishConnectionRules] VERTICAL: Both orphans - menu source becomes parent of target',
        );
        return {
          isAllowed: true,
          edgeType: 'vertical' as const,
        ...
        console.log(
          '[@hook:establishConnectionRules] VERTICAL: Both orphans - menu target becomes parent of source',
        );
        return {
          isAllowed: true,
          edgeType: 'vertical' as const,
    ...
    if (sourceIsOrphan) {
      ...
      const newParentChain = [...(targetNode.data.parent || []), targetNode.id];
      ...
      const newParentChain = [...(sourceNode.data.parent || []), sourceNode.id];
      ...
      console.log(
        '[@hook:establishConnectionRules] VERTICAL: Unexpected case - both nodes have parents',
      );
      return {
        isAllowed: true,
        edgeType: 'vertical' as const,
    ...
    if (!sourceIsOrphan && !targetIsOrphan) {
      // Both nodes already have parents - don't modify either
      console.log(
        '[@hook:establishConnectionRules] Both nodes have parents - preserving existing relationships',
      );
      return {
        isAllowed: true,
    ...
    if (sourceIsOrphan && targetIsOrphan) {
      ...
      if (sourceIsMenu && !targetIsMenu) {
        // Source (menu) becomes parent of target
        console.log(
          '[@hook:establishConnectionRules] Both orphans - menu source becomes parent of target',
        );
        return {
          isAllowed: true,
          edgeType: 'horizontal' as const,
        ...
        // Target (menu) becomes parent of source
        console.log(
          '[@hook:establishConnectionRules] Both orphans - menu target becomes parent of source',
        );
        return {
          isAllowed: true,
          edgeType: 'horizontal' as const,
        ...
        console.log(
          '[@hook:establishConnectionRules] Both orphans, no menu priority - creating sibling connection',
        );
        return {
          isAllowed: true,
          edgeType: 'horizontal' as const,
    ...
    console.log(
      '[@hook:establishConnectionRules] Direction unclear, defaulting to vertical connection - creating parent-child relationship',
    );
