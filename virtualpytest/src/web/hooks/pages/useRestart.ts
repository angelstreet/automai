...
export const useRestart = ({ host, device }: UseRestartParams): UseRestartReturn => {
  ...
  // Fetch 5-minute historical images (simplified)
    ...
    try {
      ...
      const response = await fetch('/server/rec/getRestartImages', {
        ...
        body: JSON.stringify({
          ...
          limit: 20, // Lazy load first 20 frames
        }),
      });
      ...
      if (data.success && data.frames) {
        console.log(
          `[@hook:useRestart] Successfully loaded ${data.frames.length} restart frames (${data.total_available} total available)`,
        );
