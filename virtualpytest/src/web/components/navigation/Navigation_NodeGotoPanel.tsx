...
export const NodeGotoPanel: React.FC<NodeGotoPanelProps> = ({
  ...
  const loadNavigationPreview = async () => {
    ...
    try {
      console.log(
        `[@component:NodeGotoPanel] Loading navigation preview for node: ${selectedNode.id}`,
      );

      // Use relative URL that goes through Vite proxy
      const url = new URL(
        `/server/navigation/preview/${treeId}/${selectedNode.id}`,
    ...
    >
      ...
      >
        <Button
          ...
          color="primary"
          startIcon={
            isExecuting ? <CircularProgress size={16} color="inherit" /> : <PlayArrowIcon />
          }
