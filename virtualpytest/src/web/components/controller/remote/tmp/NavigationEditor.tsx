...
import {
  getDeviceRemoteConfig,
  extractConnectionConfigForAndroid,
  extractConnectionConfigForIR,
  extractConnectionConfigForBluetooth,
} from '../utils/deviceRemoteMapping';
...
const NavigationEditorContent: React.FC = () => {
  ...
  const fetchDevices = useCallback(async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        console.log(
          `[@component:NavigationEditor] Successfully loaded ${registeredDevices.length} registered devices`,
        );
      } else {
  ...
  useEffect(() => {
    ...
    console.log(
      `[@component:NavigationEditor] Auto-populating connection form for device: ${selectedDeviceData.name}`,
      deviceRemoteConfig,
    );
    console.log(
      `[@component:NavigationEditor] Device ${selectedDeviceData.name} has remote type: ${deviceRemoteConfig.type}`,
    );

    if (hasAVCapabilities) {
      console.log(
        `[@component:NavigationEditor] Device ${selectedDeviceData.name} has AV capabilities for screen definition`,
      );
    }
  ...
  useEffect(() => {
    ...
    console.log(
      `[@component:NavigationEditor] Control released, disconnecting from ${remoteConfig.type} device`,
    );
    console.log(
      `[@component:NavigationEditor] Disconnection will be handled by the generic remote component`,
    );
    ...
    (device: string | null) => {
      ...
      if (device) {
        ...
        if (host) {
          ...
          console.log(
            `[@component:NavigationEditor] Selected host in context: ${host.name} (${host.local_ip}:${host.client_port})`,
          );
        }
      } else {
  ...
  const handleTakeControl = useCallback(async () => {
    ...
    if (wasControlActive && selectedDeviceData && hasAVCapabilities) {
      console.log(
        '[@component:NavigationEditor] Releasing control, checking stream status before disconnect...',
      );

      try {
        // Check current stream status while SSH connection is still active
        const response = await fetch(
          buildApiUrl('/api/virtualpytest/screen-definition/stream/status'),
        );
        if (response.ok) {
          const data = await response.json();
          if (data.success && !data.is_active) {
            console.log(
              '[@component:NavigationEditor] Stream was stopped, will restart after disconnect...',
            );

            // Restart the stream before disconnecting
            const restartResponse = await fetch(
              buildApiUrl('/api/virtualpytest/screen-definition/stream/restart'),
              {
                method: 'POST',
              },
            );
            ...
            if (restartResponse.ok) {
              ...
              if (restartData.success) {
                console.log(
                  '[@component:NavigationEditor] Stream restarted successfully before releasing control',
                );
              }
            }
          } else if (data.success && data.is_active) {
            console.log(
              '[@component:NavigationEditor] Stream is already running, no restart needed',
            );
          }
        } else {
          console.log(
            '[@component:NavigationEditor] Stream status check failed, SSH connection may not be available',
          );
        }
      } catch (error) {
        console.log(
          '[@component:NavigationEditor] Could not check/restart stream before disconnect:',
          error,
        );
  ...
  const handleTakeScreenshot = async () => {
    if (!selectedDevice || !isControlActive || !selectedNode) {
      console.log(
        '[@component:NavigationEditor] Cannot take screenshot: no device selected, not in control, or no node selected',
      );
      return;
    }
    ...
    try {
      ...
      console.log(
        `[@component:NavigationEditor] Taking screenshot for device: ${selectedDevice}, parent: ${parentName}, node: ${nodeName}`,
      );
      ...
      if (response.ok) {
        ...
        if (data.success) {
          ...
          const screenshotUrl = buildApiUrl(
            `/api/virtualpytest/screen-definition/images?path=${encodeURIComponent(screenshotPath)}`,
          );
          ...
          console.log(
            `[@component:NavigationEditor] Updated node ${selectedNode.id} with screenshot: ${screenshotUrl}`,
          );
        ...
        console.error(
          '[@component:NavigationEditor] Screenshot failed:',
          response.status,
          response.statusText,
        );
      }
  ...
  const initializeVerificationControllers = useCallback(async () => {
    if (!selectedDevice || !isControlActive) {
      console.log(
        '[@component:NavigationEditor] Cannot initialize verification controllers: no device selected or not in control',
      );
    ...
    try {
      console.log(
        `[@component:NavigationEditor] Initializing verification controllers for device: ${selectedDevice}`,
      );

      // Take control of verification controllers
      const takeControlResponse = await fetch(
        buildApiUrl('/api/virtualpytest/verification/take-control'),
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
      ...
      if (takeControlResponse.ok) {
        ...
        console.log(
          '[@component:NavigationEditor] Verification controllers initialized:',
          controlData,
        );
        ...
        if (statusResponse.ok) {
          ...
          console.error(
            '[@component:NavigationEditor] Failed to get verification controller status',
          );
          setIsVerificationActive(false);
        }
      } else {
        console.error(
          '[@component:NavigationEditor] Failed to initialize verification controllers:',
          takeControlResponse.status,
        );
        setIsVerificationActive(false);
        setVerificationControllerStatus({
          image_controller_available: false,
      ...
      console.error(
        '[@component:NavigationEditor] Error initializing verification controllers:',
        error,
      );
      setIsVerificationActive(false);
      setVerificationControllerStatus({
  ...
  const releaseVerificationControllers = useCallback(async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.error(
          '[@component:NavigationEditor] Failed to release verification controllers:',
          response.status,
        );
      }
    } catch (error) {
      console.error(
        '[@component:NavigationEditor] Error releasing verification controllers:',
        error,
      );
    } finally {
      setIsVerificationActive(false);
  ...
  const handleVerification = useCallback(
    async (nodeId: string, verifications: any[]) => {
      if (!isVerificationActive || !verifications || verifications.length === 0) {
        console.log(
          '[@component:NavigationEditor] Cannot execute verifications: controllers not active or no verifications',
        );
        return;
      }
      ...
      try {
        console.log(
          `[@component:NavigationEditor] Executing ${verifications.length} verifications for node: ${nodeId}`,
        );
        ...
        if (response.ok) {
          ...
          const processedResults = data.results
            ? data.results.map((result: any, index: number) => {
                ...
                console.log(
                  `[@component:NavigationEditor] Processed result ${index}:`,
                  processedResult,
                );
          ...
          console.log(
            `[@component:NavigationEditor] Setting verification results:`,
            processedResults,
          );
          ...
          if (data.results) {
            ...
            console.log(
              `[@component:NavigationEditor] Verification completed: ${passed}/${total} passed`,
            );

            // Log final result based on pass condition
            const finalPassed = verificationPassCondition === 'all' ? passed === total : passed > 0;
            console.log(
              `[@component:NavigationEditor] Final result (${verificationPassCondition}): ${finalPassed ? 'PASS' : 'FAIL'}`,
            );
          }
        } else {
          console.error(
            '[@component:NavigationEditor] Verification failed:',
            response.status,
            response.statusText,
          );
          setVerificationResults([]);
  ...
  const handleUpdateNode = useCallback(
    (nodeId: string, updatedData: any) => {
      ...
      if (updatedData._validation_update && updatedData._validation_result) {
        ...
        console.log(
          `[@component:NavigationEditor] Updated node ${nodeId} with validation result: ${validationResult}`,
        );
      } else {
      ...
      console.log(
        `[@component:NavigationEditor] Updated node ${nodeId} and marked tree as having unsaved changes`,
      );
    },
  ...
  const handleUpdateEdge = useCallback(
    (edgeId: string, updatedData: any) => {
      ...
      if (updatedData._validation_update && updatedData._validation_result) {
        ...
        console.log(
          `[@component:NavigationEditor] Updated edge ${edgeId} with validation result: ${validationResult}`,
        );
      } else {
        // Regular update - use simple object spread
        setEdges((prevEdges) =>
      ...
      console.log(
        `[@component:NavigationEditor] Updated edge ${edgeId} and marked tree as having unsaved changes`,
      );
    },
        ...
        >
          {isLoading ? (
            ...
            >
              ...
              <Button variant="contained" onClick={navigateToParent} startIcon={<ArrowBackIcon />}>
            ...
            <>
              ...
              {selectedDeviceData && hasAVCapabilities && isControlActive && (
                <ScreenDefinitionEditor
                  ...
                  onDisconnectComplete={() => {
                    ...
                    console.log(
                      '[@component:NavigationEditor] Screen definition editor disconnected',
                    );
                  }}
                />
              )}
              ...
              {(() => {
                ...
                console.log(
                  `[@component:NavigationEditor] Verification display check: results.length=${verificationResults.length}, lastVerifiedNodeId=${lastVerifiedNodeId}, shouldShow=${shouldShow}`,
                );
                if (shouldShow) {
                  console.log(
                    `[@component:NavigationEditor] Rendering verification results for node:`,
                    nodes.find((n) => n.id === lastVerifiedNodeId)?.data.label,
                  );
                }
                return shouldShow;
              })() && (
                ...
                >
                  ...
                  <VerificationResultsDisplay
                    ...
                    verifications={
                      nodes.find((n) => n.id === lastVerifiedNodeId)?.data.verifications || []
                    }
