...
export function HDMIStreamPanel({
  ...
  console.log(`[@component:HDMIStreamPanel] Rendering for host: ${host?.host_name || 'none'}`);

  // Simplified state - just check if AV controller proxy is available
  const [connectionError, setConnectionError] = useState<string | null>(null);

  // Check if AV functionality is available (simplified - just check if host is provided)
  const isAvailable = host ? true : false;

  // Auto-connect if requested
  useEffect(() => {
    if (autoConnect && !isAvailable) {
      console.log(
        '[@component:HDMIStreamPanel] Auto-connect requested but AV controller not available',
      );
  ...
  return (
    ...
    >
      ...
      <Box sx={{ mb: 2 }}>
        ...
        {/* Status chips */}
        <Box sx={{ mb: 2 }}>
          <Chip label="Available" color="success" icon={<Settings />} size="small" sx={{ mr: 1 }} />
          <Chip label="Ready" color="primary" icon={<Videocam />} size="small" />
        </Box>
