  ...
  return (
    <Box sx={{ mt: 0 }}>
      ...
      <Box sx={{ 
        ...
        borderRadius: 1,
        mb: 1,
        backgroundColor: testResult.resultType === 'PASS' || testResult.success
          ? 'rgba(76, 175, 80, 0.1)' 
        ...
        <Typography sx={{ 
          fontWeight: 'bold',
          fontSize: '0.75rem',
          color: testResult.resultType === 'PASS' || testResult.success
            ? '#4caf50' 
            : testResult.resultType === 'ERROR' 
              ? '#ff9800' 
              : '#f44336'
        }}>
          {testResult.resultType === 'PASS' || testResult.success ? 'PASS' : 
           testResult.resultType === 'ERROR' ? 'ERROR' : 'FAIL'}
        </Typography>
      ...
      {verification.controller_type === 'text' && 
       (testResult.searchedText || testResult.sourceImageUrl) && (
        <VerificationTextComparisonDisplay
          searchedText={testResult.searchedText || verification.params?.reference_text || verification.inputValue || ''}
          ...
          {testResult.success && testResult.matches ? (
            <>
              <Typography variant="caption" sx={{ 
                ...
                Found {testResult.total_matches} element(s) after {testResult.wait_time?.toFixed(1)}s
              </Typography>
