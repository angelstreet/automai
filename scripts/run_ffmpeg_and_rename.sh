...
# Configuration array for grabbers: video_device:audio_device:capture_dir:fps  arecord -l
declare -A GRABBERS=(
  ["0"]="/dev/video2:hw:0,0:/var/www/html/stream/capture1:5"
  ["1"]="/dev/video0:hw:3,0:/var/www/html/stream/capture2:5"
)
...
start_grabber() {
  ...
  /usr/bin/ffmpeg -y -loglevel verbose \
    ...
    -filter_complex "[0:v]split=2[stream][capture];[stream]scale=640:360,format=yuv420p[streamout];[capture]fps=1[captureout]" \
    -map "[streamout]" -map 1:a \
    -c:v libx264 -preset ultrafast -tune zerolatency -b:v 500k -maxrate 600k -bufsize 100k \
    -g 5 -keyint_min 5 -sc_threshold 0 -flags low_delay+global_header -threads 2 \
    -c:a aac -b:a 128k \
    ...
    > "/tmp/ffmpeg_output_${index}.log" 2>&1 &
  local FFMPEG_PID=$!
  sleep 2
  ps -p $FFMPEG_PID > /dev/null || { echo "Error: FFmpeg failed for $video_device. Check /tmp/ffmpeg_output_${index}.log"; exit 1; }
