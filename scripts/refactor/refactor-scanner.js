...
function loadIgnoredPatterns() {
  ...
  try {
    ...
    console.error(
      `Error loading refactorignore from ${path.join(__dirname, 'refactorignore')}: ${error.message}`,
    );
  }
...
function main() {
  console.log(
    `${colors.cyan}=== Refactor Scanner: ${scope.toUpperCase()} Scope ===${colors.reset}\n`,
  );
  ...
  try {
    ...
    console.log(
      `${colors.yellow}Ignored directories:${colors.reset} ${ignoredPatterns.ignoredDirs.join(', ')}`,
    );
    console.log(
      `${colors.yellow}Ignored files:${colors.reset} ${ignoredPatterns.ignoredFiles.join(', ')}`,
    );
    console.log(
      `${colors.yellow}Ignored extensions:${colors.reset} ${ignoredPatterns.ignoredExtensions.join(', ')}`,
    );
    console.log(
      `${colors.yellow}Exempted patterns:${colors.reset} ${ignoredPatterns.exemptedPatterns.length} patterns\n`,
    );
    ...
    if (!fs.existsSync(targetDir)) {
      console.error(
        `${colors.red}Error:${colors.reset} Target directory ${targetDir} does not exist`,
      );
      process.exit(1);
    }
