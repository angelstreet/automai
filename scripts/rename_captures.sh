  ...
  while read -r dir events file; do
    if [[ "$file" =~ ^test_capture_[0-9]+\.jpg$ ]]; then
      ...
      if [ -f "$filepath" ]; then
        ...
        timestamp=$(TZ="Europe/Zurich" stat -c %y "$filepath" 2>/dev/null | awk '{print $1$2}' | sed 's/[-:]//g;s/\..*//')
        ms=$(TZ="Europe/Zurich" stat -c %y "$filepath" 2>/dev/null | awk '{print $2}' | sed 's/.*\.//;s/[^0-9]//g' | cut -c 1-3)
