...
const DeploymentWizardStep2Client: React.FC<DeploymentWizardStep2Props> = ({
  ...
  const ErrorIndicator = () => {
    ...
    const isRateLimitError =
      scriptsError?.includes('API rate limit reached') || scriptsError?.includes('403');

    return (
      <div
        className={`${isRateLimitError ? 'bg-orange-50 dark:bg-orange-900/20 border-orange-200 dark:border-orange-800' : 'bg-red-50 dark:bg-red-900/20 border-red-200 dark:border-red-800'} border rounded-md p-4 my-4`}
      >
        <div className="flex">
          <div className="flex-shrink-0">
            <svg
              className={`h-5 w-5 ${isRateLimitError ? 'text-orange-400' : 'text-red-400'}`}
              viewBox="0 0 20 20"
              fill="currentColor"
            >
              <path
          ...
          <div className="ml-3">
            <h3
              className={`text-sm font-medium ${isRateLimitError ? 'text-orange-800 dark:text-orange-200' : 'text-red-800 dark:text-red-200'}`}
            >
              {isRateLimitError ? 'GitHub API Rate Limit' : 'Repository Error'}
            </h3>
            <div
              className={`mt-2 text-sm ${isRateLimitError ? 'text-orange-700 dark:text-orange-300' : 'text-red-700 dark:text-red-300'}`}
            >
              <p>{scriptsError || 'Failed to load scripts from repository'}</p>
              {isRateLimitError && (
                <p className="mt-1">
                  GitHub limits API requests to prevent abuse. Try again later or select a different
                  repository.
                </p>
              )}
            </div>
