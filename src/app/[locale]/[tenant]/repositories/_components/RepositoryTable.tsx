...
export function RepositoryTable({
  ...
  function getProviderColor(providerId: string, providers: GitProvider[]): string {
    ...
    return provider.type === 'github' ? '#24292e' : 
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex flex-wrap gap-2">
            {selectedProviders.map(providerName => {
              ...
              const color = provider?.type === 'github' ? '#24292e' : 
                            provider?.type === 'gitlab' ? '#fc6d26' : 
                            provider?.type === 'gitea' ? '#609926' : '#4285f4';
        ...
        <div className="border rounded-lg overflow-hidden">
          <table className="min-w-full divide-y divide-border">
            <thead className="bg-muted">
              <tr>
                <th className="px-6 py-3 text-left text-xs font-medium text-muted-foreground uppercase tracking-wider">{t('repository')}</th>
                <th className="px-6 py-3 text-left text-xs font-medium text-muted-foreground uppercase tracking-wider">{t('provider')}</th>
                <th className="px-6 py-3 text-left text-xs font-medium text-muted-foreground uppercase tracking-wider">{t('status')}</th>
                <th className="px-6 py-3 text-left text-xs font-medium text-muted-foreground uppercase tracking-wider">{t('updatedAt')}</th>
              ...
              {filteredRepos.map(repo => {
                ...
                return (
                  <tr key={repo.id} className="hover:bg-muted/30">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <Badge variant={repo.syncStatus === 'SYNCED' ? 'secondary' : 
                                      repo.syncStatus === 'ERROR' ? 'destructive' : 
                                      'outline'}>
                        {repo.syncStatus}
                      </Badge>
                    </td>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                      <div className="flex items-center justify-end gap-2">
                        ...
                        >
                          <RefreshCw className={`h-4 w-4 ${syncingRepoId === repo.id ? 'animate-spin' : ''}`} />
                        </Button>
