  ...
  const sortUsecases = (usecases: UseCase[]) => {
    return [...usecases].sort((a, b) => {
      ...
      if (sortConfig.key === "lastModified" || sortConfig.key === "createdAt") {
        ...
        comparison = (a[sortConfig.key as keyof UseCase] || "").toString().localeCompare((b[sortConfig.key as keyof UseCase] || "").toString());
      }
          ...
          <>
            {getFavoriteUseCases().length > 0 && (
              <div className="mb-6 bg-background dark:bg-background rounded-lg shadow border border-border">
                <div className="px-4 py-2 bg-muted dark:bg-muted/80 border-b border-border">
                  <h3 className="font-semibold text-foreground dark:text-foreground">★ Favorite Use Cases</h3>
                </div>
                <TableHeader />
            ...
            <div className="space-y-4">
              {projects.map((project) => (
                <div key={project.id} className="bg-background dark:bg-background rounded-lg shadow border border-border">
                  <button
            ...
            {isCreateDialogOpen && (
              <div className="fixed inset-0 bg-black/50 dark:bg-black/80 flex items-center justify-center z-[49]">
                <div className="bg-background dark:bg-background/95 border border-border dark:border-border/80 w-[500px] rounded-lg shadow-lg dark:shadow-2xl relative z-[50] select-none">
                  <div className="p-6">
                    <h2 className="text-xl font-bold mb-4 text-foreground dark:text-foreground/90">New Use Case</h2>
                    <div className="space-y-4">
                      <Select 
                      ...
                      >
                        ...
                        <SelectContent position="popper" className="z-[51] dark:bg-background/95">
                          {projects.map((p) => (
                            <SelectItem key={p.id} value={p.id} className="dark:text-foreground/90">{p.name}</SelectItem>
                          ))}
                        </SelectContent>
                      </Select>
                      ...
                      >
                        ...
                        <SelectContent className="dark:bg-background/95">
                          <SelectItem value="web" className="dark:text-foreground/90">Web 🌐</SelectItem>
                          <SelectItem value="android" className="dark:text-foreground/90">Android 📱</SelectItem>
                          <SelectItem value="ios" className="dark:text-foreground/90">iOS 📱</SelectItem>
                          <SelectItem value="desktop" className="dark:text-foreground/90">Desktop 💻</SelectItem>
                          <SelectItem value="api" className="dark:text-foreground/90">API 🔌</SelectItem>
                          <SelectItem value="python" className="dark:text-foreground/90">Python 🐍</SelectItem>
                        </SelectContent>
                      </Select>

                      <div className="space-y-2">
                        <label className="text-sm text-muted-foreground dark:text-muted-foreground/90">Description (Optional)</label>
                        <textarea 
