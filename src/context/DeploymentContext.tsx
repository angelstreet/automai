...
import { getDeployments, getDeploymentById, createDeployment as createDeploymentAction, abortDeployment as abortDeploymentAction, refreshDeployment as refreshDeploymentAction, updateDeployment, deleteDeployment, getScriptsForRepository } from '@/app/[locale]/[tenant]/deployment/actions';
...
export const DeploymentProvider: React.FC<{ children: ReactNode; userData?: AuthUser | null }> = ({ children, userData }) => {
    ...
    if (typeof window !== 'undefined') {
      try {
        ...
        if (cachedData) {
          ...
          console.log('[DeploymentContext] Using initial cached deployment data:', parsedData.deployments?.length || 0);
    ...
    try {
      ...
      if (user) {
        log('[DeploymentContext] Using existing user data:', {
          ...
          source: user === userContext.user 
            ? 'UserContext' 
            : (user === state.currentUser ? 'state' : 'props')
        });
  ...
  const fetchRepositories = useCallback(async () => {
    ...
    try {
      ...
      if (result && Array.isArray(result)) {
        safeUpdateState(setState, state, { ...state, repositories: result }, 'repositories-updated');
        try {
  ...
  const fetchDeployments = useCallback(async (forceFresh = false) => {
    ...
    return await protectedFetch('fetchDeployments', async () => {
      try {
        ...
        setState(prev => ({ ...prev, loading: prev.deployments.length === 0, isRefreshing: true, error: null }));
        ...
        try {
          const updatedState = { ...state, deployments: deployments || [], loading: false, isRefreshing: false, isInitialized: true };
          localStorage.setItem('cached_deployment', JSON.stringify(updatedState));
          localStorage.setItem('cached_deployment_time', Date.now().toString());
        } catch (e) {
        ...
        setState(prev => ({ ...prev, loading: false, isRefreshing: false, error: err.message || 'Failed to fetch', isInitialized: true }));
        return null;
      }
    });
  ...
  const createDeployment = useCallback(async (formData: DeploymentFormData) => {
    const result = await protectedFetch('createDeployment', async () => {
      try {
        ...
        const apiResult = await createDeploymentAction(formData) as { success: boolean; data?: { id: string }; error?: string };
    ...
    const result = await protectedFetch(`refreshDeployment-${id}`, async () => {
      try {
        ...
        if (result?.deployment) {
          safeUpdateState(setState, state, {
            ...state,
            deployments: state.deployments.map(d => d.id === id && result.deployment ? result.deployment : d),
          }, `deployment-${id}`);
  ...
  const fetchDeploymentStatus = useCallback(async (id: string) => {
    const result = await protectedFetch(`fetchDeploymentStatus-${id}`, async () => {
      ...
      if (refreshResult.success && refreshResult.deployment) {
        return { status: refreshResult.deployment.status, progress: 0, deployment: refreshResult.deployment };
      }
  ...
  const deleteDeploymentImpl = useCallback(async (id: string) => {
    const result = await protectedFetch(`deleteDeployment-${id}`, async () => {
      try {
        ...
        safeUpdateState(setState, state, { ...state, deployments: filteredDeployments }, `delete-deployment-${id}-optimistic`);
        const success = await deleteDeployment(id);
        if (success) {
          console.log(`[DeploymentContext] Successfully deleted deployment ${id}, forcing refresh`);
          safeUpdateState(setState, state, { ...state, loading: true, isRefreshing: true }, `delete-deployment-${id}-loading`);
          setTimeout(() => fetchDeployments(true), 300);
          return { success: true };
        } else {
          safeUpdateState(setState, state, { ...state, deployments: originalDeployments }, `delete-deployment-${id}-restore`);
  ...
  const contextValue = useMemo(() => ({
    ...
    refreshUserData: async () => { await refreshUserData(); return state.currentUser; },
...
export function useDeployment() {
  ...
  if (!context) {
    console.warn('[useDeployment] Deployment context is null, returning fallback. This should not happen if using the centralized context system.');
    return {
      deployments: [],
      repositories: [],
      loading: true,
