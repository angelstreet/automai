...
export function CICDProvider({ children }: { children: ReactNode }) {
  // Use SWR hooks
  const { data: providersData, error: providersError, mutate: mutateProviders, isValidating: isRefreshingProviders } = useCICDProviders();
  ...
  const loading = useMemo(() => 
    (providersData === undefined && !providersError) ||
    (jobsData === undefined && !jobsError),
    [providersData, providersError, jobsData, jobsError]
  );
  
  const error = useMemo(() => 
    providersError ? String(providersError) :
    jobsError ? String(jobsError) : null,
  ...
  const contextValue = useMemo((): CICDContextType => ({
    // State
    providers,
    jobs,
    builds: [],
