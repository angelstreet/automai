    ...
    <div style={{ display: 'flex' }}>
      ...
      {deploymentDetails && (
        <div style={{ flex: 1, padding: '10px', border: '1px solid #ccc' }}>
          ...
          <div><strong>ID:</strong> {deploymentDetails.id}</div>
          <div><strong>Name:</strong> {deploymentDetails.name}</div>
          <div><strong>Description:</strong> {deploymentDetails.description}</div>
          <div><strong>Status:</strong> {deploymentDetails.status}</div>
          <div><strong>Repository ID:</strong> {deploymentDetails.repositoryId}</div>
          <div><strong>Created:</strong> {new Date(deploymentDetails.createdAt).toLocaleString()}</div>
          {deploymentDetails.startedAt && (
            <div><strong>Started:</strong> {new Date(deploymentDetails.startedAt).toLocaleString()}</div>
          )}
          {deploymentDetails.completedAt && (
            <div><strong>Completed:</strong> {new Date(deploymentDetails.completedAt).toLocaleString()}</div>
          )}
