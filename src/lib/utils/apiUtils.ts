  ...
  for (let attempt = 0; attempt <= retryAttempts!; attempt++) {
    try {
      ...
      if (response.status === 429) {
        ...
        console.info(`Rate limited (429) on ${url}, retrying after ${retryAfterMs}ms`);
...
export async function apiRequest<T = any>(
  ...
  const { baseUrl = DEFAULT_OPTIONS.baseUrl, cacheKey, cacheTtl, ...fetchOptions } = options;
  ...
  if (cacheKey) {
    ...
    if (cachedData) {
      console.debug(`Using cached data for ${fullUrl}`);
      return cachedData;
    }
  }
  ...
  try {
    ...
    if (!response.ok) {
      ...
      try {
        ...
        errorMessage = `Request failed with status ${e}`;
      }
    ...
    console.error(`API request failed for ${fullUrl}`, { error: error.message });
    throw error;
  }
}
 ...
 * Make a GET request
 */
export async function get<T = any>(url: string, options: ApiRequestOptions = {}): Promise<T> {
 ...
 */
export async function del<T = any>(url: string, options: ApiRequestOptions = {}): Promise<T> {
  return apiRequest<T>(url, {
    ...options,
    method: 'DELETE',
  });
