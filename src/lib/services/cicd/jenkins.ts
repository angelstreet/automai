...
export class JenkinsProvider implements CICDProvider {
  ...
  initialize(config: CICDProviderConfig): void {
    ...
    console.log(
      `[JENKINS] Initializing provider with config: url=${this.baseUrl}, type=${config.type}`,
    );
  ...
  private async getCrumb(): Promise<CICDResponse<{ crumb: string; crumbRequestField: string }>> {
    try {
      ...
      const result = await this.jenkinsRequest<any>('/crumbIssuer/api/json', {
        method: 'GET',
        headers: {
      ...
      console.log(
        `[JENKINS] CSRF crumb obtained: ${crumbData.crumbRequestField}=${crumbData.crumb.substring(0, 10)}...`,
      );
  ...
  private async jenkinsRequest<T>(
    ...
    try {
      ...
      if (['POST', 'PUT', 'DELETE'].includes(options.method || '')) {
        ...
        if (!this.crumb) {
          ...
          if (crumbResult.success && crumbResult.data) {
            ...
            console.warn(
              '[JENKINS] Failed to get CSRF crumb, proceeding without it:',
              crumbResult.error,
            );
        ...
        if (this.crumb) {
          ...
          console.log(
            `[JENKINS] Added CSRF crumb to request headers: ${this.crumb.crumbRequestField}=${this.crumb.crumb.substring(0, 10)}...`,
          );
        } else {
      ...
      if (contentType?.includes('application/json')) {
        ...
        console.log(
          `[JENKINS] Text response (first 500 chars): ${data.substring(0, 500)}${data.length > 500 ? '...' : ''}`,
        );
      }
  ...
  async getAvailableJobs(): Promise<CICDResponse<CICDJob[]>> {
    try {
      ...
      const result = await this.jenkinsRequest<any>(
        '/api/json?tree=jobs[name,url,description,buildable]',
      );
  ...
  async getJobDetails(jobId: string): Promise<CICDResponse<CICDJob>> {
    try {
      ...
      const result = await this.jenkinsRequest<any>(
        `/job/${encodeURIComponent(jobId)}/api/json?tree=name,url,description,property[parameterDefinitions[name,type,defaultParameterValue[value],description,choices]]`,
      );
      ...
      const paramProperty = result.data.property?.find(
        (prop: any) => prop?.parameterDefinitions && Array.isArray(prop.parameterDefinitions),
      );
  ...
  async triggerJob(
    ...
    console.log(
      `[JENKINS] Triggering job '${jobId}' with parameters:`,
    ...
    try {
      ...
      if (parameters && Object.keys(parameters).length > 0) {
        console.log(
          `[JENKINS] Job has ${Object.keys(parameters).length} parameters, using buildWithParameters endpoint`,
        );
        ...
        const result = await this.jenkinsRequest<any>(endpoint, { method: 'POST' });
      ...
      while (!buildId && attempts < 10) {
        ...
        if (queueResult.success && queueResult.data.executable) {
          ...
          console.log(
            `[JENKINS] Build not started yet, queue item status:`,
            queueResult.data.blocked
              ? 'blocked'
              : queueResult.data.stuck
                ? 'stuck'
                : queueResult.data.cancelled
                  ? 'cancelled'
                  : 'waiting',
          );
        } else {
...
${stages}
  ...
  async getBuildStatus(jobId: string, buildId: string): Promise<CICDResponse<CICDBuild>> {
    try {
      ...
      const result = await this.jenkinsRequest<any>(
        `/job/${encodeURIComponent(jobId)}/${buildId}/api/json?tree=id,building,result,timestamp,url`,
      );
  ...
  async getBuildLogs(jobId: string, buildId: string): Promise<CICDResponse<string>> {
    try {
      ...
      const result = await this.jenkinsRequest<string>(
        `/job/${encodeURIComponent(jobId)}/${buildId}/consoleText`,
      );
  ...
  async deleteJob(jobId: string, folderPath?: string): Promise<CICDResponse<boolean>> {
    try {
      console.log(
        `[JENKINS] Deleting job '${jobId}'${folderPath ? ` in folder '${folderPath}'` : ''}`,
      );
