...
export function detectProviderFromUrl(url: string): GitProviderType {
  ...
  try {
    ...
    // Check for GitHub
    if (normalizedUrl.startsWith('github.com/') || normalizedUrl.startsWith('www.github.com/')) {
      return 'github';
    }

    // Check for GitLab
    if (normalizedUrl.startsWith('gitlab.com/') || normalizedUrl.startsWith('www.gitlab.com/')) {
      return 'gitlab';
    }

    // Check for known Gitea instances
    if (normalizedUrl.includes('gitea.') || normalizedUrl.includes('/gitea/')) {
      return 'gitea';
    }
