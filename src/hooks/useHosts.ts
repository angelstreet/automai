...
const hostsFetcher = async () => {
  ...
  if (requestTimestamps.length > REQUEST_LIMIT) {
    console.warn(
      `Too many API requests (${requestTimestamps.length}) in ${REQUEST_WINDOW / 1000}s window. Throttling.`,
    );
...
export function useHosts(initialHosts: Host[] = []) {
  ...
  const {
    ...
    isValidating,
    mutate,
  } = useSWR(HOSTS_CACHE_KEY, throttledFetcher, {
  ...
  const updateHostDetails = useCallback(
    async (id: string, updates: Partial<Omit<Host, 'id'>>) => {
      try {
        // Optimistic update
        mutate(
          (currentHosts) =>
            currentHosts?.map((host) => (host.id === id ? { ...host, ...updates } : host)),
          false,
        );
  ...
  const deleteHost = useCallback(
    async (id: string) => {
      try {
        ...
        mutate((currentHosts) => currentHosts?.filter((host) => host.id !== id), false);
  ...
  const testHostConnection = useCallback(
    async (id: string) => {
      try {
        ...
        mutate(
          (currentHosts) =>
            currentHosts?.map((h) => (h.id === id ? { ...h, status: 'failed' } : h)),
          false,
        );
        ...
        mutate(
          (currentHosts) =>
            currentHosts?.map((h) => (h.id === id ? { ...h, status: 'testing' } : h)),
          false,
        );
