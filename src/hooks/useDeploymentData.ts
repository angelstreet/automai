...
export function useDeployments() {
  return useSWR('deployments', () => actionFetcher(getDeployments), {
...
export function useAvailableHosts() {
  return useSWR('hosts', () => actionFetcher(getHosts), {
    dedupingInterval: 15 * 60 * 1000, // 15 minutes
    revalidateOnFocus: false,
  });
}
...
export function useRepositoriesForDeployment() {
  return useSWR('repositories-for-deployment', () => actionFetcher(getRepositories), {
    dedupingInterval: 15 * 60 * 1000, // 15 minutes
    revalidateOnFocus: false,
  });
}
  ...
  try {
    ...
    if (result.success && result.deployment) {
      ...
      await mutate(
        ...
        (data: Deployment[] | undefined) => {
          if (!data) return data;
          return data.map((deployment) => (deployment.id === id ? result.deployment : deployment));
        },
        false,
      );
    }
