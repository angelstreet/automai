...
import {
  getCICDProvidersByTeam,
  createCICDProviderWithTeam,
  updateCICDProviderTeam,
} from '@/app/actions/cicd-team';
...
export function useCICDTeamIntegration() {
   ...
   */
  const createTeamCICDProvider = useCallback(
    async (providerData: Omit<CICDProvider, 'id' | 'created_at' | 'updated_at'>) => {
      if (!selectedTeam) {
        toast({
          title: 'No team selected',
   ...
   */
  const moveCICDProviderToTeam = useCallback(
    async (providerId: string, teamId: string) => {
      try {
